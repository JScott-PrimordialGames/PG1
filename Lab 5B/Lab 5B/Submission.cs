...
namespace FSPG1
{
    class Submission
    {
        ...
        */
        ...
        {
            double initpoint = start / factor;
            int startpoint = (int) Math.Floor(initpoint);

            double stoppoint = end / factor;
            int endpoint = (int)Math.Ceiling(stoppoint);

            int count = 0;

            for (int i = startpoint; i <= endpoint; i++)
            {
                int product = i * factor;
